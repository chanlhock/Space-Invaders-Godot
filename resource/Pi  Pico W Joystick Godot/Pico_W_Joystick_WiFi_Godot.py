...
# Direct UDP to Godot - No bridge required!
...
GODOT_IP = "192.168.0.23"  # Replace with your computer/Godot IP address
GODOT_PORT = 9000           # Port Godot is listening on
...
def send_to_godot(sock, target_ip, target_port, x_val, y_val, button_val):
    """Send joystick data directly to Godot"""
    try:
        # Calculate percentages for Godot
...
# Main program
print("\n" + "="*50)
print("PICO W JOYSTICK - DIRECT TO GODOT")
print("="*50)
...
if not wifi_connected:
    ...
    if init_udp_socket():
        print(f"Sending directly to Godot at {GODOT_IP}:{GODOT_PORT}")
        update_display("Direct to Godot", f"→ {GODOT_IP}", f"Port: {GODOT_PORT}", "Move joystick")
